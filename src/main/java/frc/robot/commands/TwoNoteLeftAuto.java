...
public class TwoNoteLeftAuto extends SequentialCommandGroup {
  /** Creates a new TwoNoteLeftAuto. */
  public TwoNoteLeftAuto(SwerveDrive driveSwerve, Joystick driverL, Joystick driverR, Shooter noteShooter, Intake inTake, Pivot shooterPivot) {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    addCommands(
      new InstantCommand(() -> driveSwerve.zeroGyroAuto(56)),
      (new PivotHoldCommand(shooterPivot, Constants.backshotPos).alongWith(new SpinUpShooter(noteShooter, Constants.shooterHighSpeed, 0)).alongWith(new intakeSpin(inTake, 0))).raceWith(new WaitCommand(2)),
